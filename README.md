...
- 📋 **공부 로드맵 탭 (페이즈 탭형)** — **단계별 학습 순서**(DB → 알고리즘 → CS → 아키텍처 → **Java/Kotlin**). 5개 페이즈 · **31개 스테이지** · 232개 토픽 + 153개 실습(hands-on) + 페이즈별 통과 기준(stage gate). **상단 페이즈 버튼으로 분야를 전환**(한 번에 한 페이즈만 표시 → 페이즈 간 스크롤 없음, 마지막 본 페이즈 기억). 스테이지를 누르면 **별도 학습 창([`learn.html`](src/dashboard/learn.html))**이 팝업으로 떠서 학습(Ctrl/⌘+클릭은 새 탭) → **✅ 학습 완료** / **📋 Claude 퀴즈용 복사**(Chrome의 Claude/claude.ai에 붙여넣어 그 단계 퀴즈 생성). 출처: CS = gyoogle *'면접을 위한 CS 전공지식 노트'*, 알고리즘 = 프로그래머스 레벨(Lv0~5) + LeetCode, DB = 현행 + SQLD 보강, Java/Kotlin = 동시성·코루틴 / 언어 내부 / Spring 내부. (서술형 원문: [`STUDY_ROADMAP.md`](STUDY_ROADMAP.md))
...
└── src/
    ...
    └── dashboard/             # 탭형 대시보드
        ...
        ├── learn.js
        ├── stats.js           # 📊 통계 탭
        ├── roadmap.js         # 📋 로드맵 탭(페이즈 탭형 → learn.html 별도 창)
